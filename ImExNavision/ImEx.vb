...
#End Region
    ...
    Private Sub ExportInvoices()
        ...
        While dtr.Read
            ...
            ExecuteNavSQL("Insert into ""POS Sales Header"" (InvNo,Date,OrdNo,CustId,AgentId,Void,SubTotal,GSTAmt,PaidAmt,Payterms) Values (" & _
            SafeSQL(dtr("InvNo").ToString) & "," & Format(dtr("InvDt"), "yyyy-MM-dd") & "', " & SafeSQL(dtr("OrdNo").ToString) & _
            SafeSQL(dtr("CustId").ToString) & "," & SafeSQL(dtr("AgentId").ToString) & "," & SafeSQL(dtr("VOID").ToString) & _
            SafeSQL(dtr("SubTotal").ToString) & "," & SafeSQL(dtr("GSTAmt").ToString) & "," & _
            SafeSQL(dtr("PaidAmt").ToString) & "," & SafeSQL(dtr("PayTerms").ToString))
    ...
    Private Sub ExportInvItem()
        ...
        Try
            ...
            While dtr1.Read
                ...
                Dim sSQL As String = "Insert into ""POS Sales Line"" (InvNo,ItemNo,UOM,Qty,Price,Discount,SubAmt,SalesType) Values (" & _
                SafeSQL(dtr1("InvNo").ToString) & "," & SafeSQL(dtr1("ItemNo").ToString) & "," & _
                SafeSQL(dtr1("UOM").ToString) & "," & SafeSQL(dtr1("Qty").ToString) & ",'" & SafeSQL(dtr1("Price").ToString) & "," & SafeSQL(dtr1("Discount").ToString) & "," & dtr1("SubAmt") & "," & dtr1("SalesType")
